...
if selected_menu_item == "Search":
    ...
    connection = sql.connect(host='localhost', user='root', password=<your_password>, database='PhonepePulse')

    table_mapping = {
        "Statewise Various Payment Type of Transaction Amount and Count": "agg_trans",
        "Statewise User Count by Mobile Brand": "agg_user",
        "Districtwise Total Transaction Count and Amount": "map_trans",
        "Districtwise Registered User Count": "map_user",
        "Top 10 States with Pincodes of Total Transaction Amount and Count": "top_trans",
        "Top 10 States with Pincodes of Registered Users": "top_user"
    }
...
if selected_menu_item == "Insights":
    ...
    if selected_table in table_names:
        ...
        if selected_display_name == "Statewise Various Payment Type of Transaction Amount and Count":
            ...
            transaction_names = (df['Transaction_type'].unique())
            ...
            filtered_df = df[(df['Year'] == selected_year) & (df['Quarter'] == selected_quarter) & (df['Transaction_type'] == selected_transaction)]

            # Visualize data using charts
            chart_type = st.sidebar.selectbox('Select Chart Type', ['Bar Chart', 'Pie Chart'])

            if chart_type == 'Bar Chart':
                fig = px.bar(filtered_df, x='State', y='Transaction_amount', color='State', title='Total Amount by State')
                ...
                fig = px.pie(filtered_df, values='Transaction_count', names='State', title='Total Count by State')
                fig.update_layout(height=750, width=1000, title_font=dict(size=24)) 
                st.plotly_chart(fig, use_container_width=True)
        ...
        if selected_display_name == "Statewise User Count by Mobile Brand":
            ...
            selected_year = st.sidebar.selectbox('Select Year', years)
            selected_quarter = st.sidebar.selectbox('Select Quarter', quarters)
            selected_brand = st.sidebar.selectbox('Select Mobile Brands', brands)

            # Filter data based on selected options
            filtered_df = df[(df['Year'] == selected_year) & (df['Quarter'] == selected_quarter) & (df['Brands'] == selected_brand)]

            # Visualize data using charts
            chart_type = st.sidebar.selectbox('Select Chart Type', ['Bar Chart', 'Pie Chart'])

            if chart_type == 'Bar Chart':
                fig = px.bar(filtered_df, x='State', y='Count', color='State', title='User Count by State')
                fig.update_layout(height=750, width=1000,title_font=dict(size=24)) 
                st.plotly_chart(fig, use_container_width=True)
        ...
        if selected_display_name == "Districtwise Total Transaction Count and Amount":
            ...
            if chart_type == 'Bar Chart':
                fig = px.bar(filtered_df, x='District', y='Amount', color='District', title='Total Amount by District')
                fig.update_layout(height=750, width=1000,title_font=dict(size=24)) 
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("""<span style="font-size: 24px;"> By selecting "Bar Chart" as the chart type in the sidebar.
                ...
                fig = px.pie(filtered_df, values='Count', names='District', title='Total Count by District')
                fig.update_layout(height=750, width=1000,title_font=dict(size=24)) 
                st.plotly_chart(fig, use_container_width=True)
                st.markdown("""<span style="font-size: 24px;"> By selecting "Pie Chart" as the chart type in the sidebar.
        ...
        if selected_display_name == "Districtwise Registered User Count":
            ...
            if chart_type == 'Bar Chart':
                fig = px.bar(filtered_df, x='District', y='RegisteredUser', color='District', title='Registered Users by District')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;"> By selecting "Bar Chart" as the chart type in the sidebar. 
                ...
                fig = px.pie(filtered_df, values='AppOpens', names='District', title='App Opened by District')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;">By selecting "Pie Chart" as the chart type in the sidebar.
        ...
        if selected_display_name == "Top 10 States with Pincodes of Total Transaction Amount and Count":
            ...
            state_amounts = filtered_df.groupby('State')['Transaction_amount'].sum().reset_index()
            state_amounts = state_amounts.sort_values('Transaction_amount', ascending=False)
            ...
            if chart_type == 'Bar Chart':
                fig = px.bar(state_filtered_df, x='Pincode', y='Transaction_amount', color='Pincode', title=f'Total Amount by Pincode in {selected_state}')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;"> By selecting "Bar Chart" as the chart type in the sidebar. 
                ...
                fig = px.pie(state_filtered_df, values='Transaction_count', names='Pincode', title=f'Total Count by Pincode in {selected_state}')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;">By selecting "Pie Chart" as the chart type in the sidebar. 
        ...
        if selected_display_name == "Top 10 States with Pincodes of Registered Users":
            ...
            state_users = filtered_df.groupby('State')['RegisteredUsers'].sum().reset_index()
            state_users = state_users.sort_values('RegisteredUsers', ascending=False)
            ...
            if chart_type == 'Bar Chart':
                fig = px.bar(state_filtered_df, x='Pincode', y='RegisteredUsers', color='Pincode', title=f'Registered Users by Pincode in {selected_state}')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;"> By selecting "Bar Chart" as the chart type in the sidebar.
                ...
                fig = px.pie(state_filtered_df, values='RegisteredUsers', names='Pincode', title=f'Registered Users by Pincode in {selected_state}')
                st.plotly_chart(fig, use_container_width=True)
                fig.update_layout(height=750, width=1000,title_font=dict(size=24))
                st.markdown("""<span style="font-size: 24px;">By selecting "Pie Chart" as the chart type in the sidebar. 
...
if selected_menu_item == "Geo visualization":
    ...
    def fetch_data_from_database(selected_year):
        ...
        query = f"SELECT `State`, `Year`, `Quarter`, `Transaction_type`, `Transaction_count`, `Transaction_amount` FROM agg_trans WHERE `Year` = {selected_year}"
        cursor = connection.cursor()
        ...
        df_agg = df.groupby(['State', 'Year']).agg({'Transaction_count': 'sum', 'Transaction_amount': 'sum'}).reset_index()
        df_agg.rename(columns={'Transaction_count': 'Transaction count', 'Transaction_amount': 'Transaction Amount'}, inplace=True)
    ...
    geojson_data = "C:\\Users\\senth\\OneDrive\\Desktop\\project\\map.geojson.py"
